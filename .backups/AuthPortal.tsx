...
export default function AuthPortal({ onLoginSuccess, showToast }: AuthPortalProps) {
  ...
  useEffect(() => {
    ...
    if (googleAuth === 'success' && token) {
      localStorage.setItem('jiuspeak_access_token', token);
      if (userId) localStorage.setItem('jiuspeak_user_id', userId);
              ...
              <form onSubmit={handleLogin} className="space-y-4 font-sans" id="login-form">
                ...
                <div className="space-y-3">
                  <a
                    ...
                    className="w-full py-3 bg-white hover:bg-gray-50 text-gray-800 rounded-xl font-bold text-sm flex items-center justify-center gap-3 border border-gray-200 transition-all shadow-sm"
                  >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-slate-600 font-mono">ou</span>
                    <div className="flex-1 h-px bg-slate-800" />
                  </div>
                </div>
